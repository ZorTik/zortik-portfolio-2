...
export default function Dropdown({button, label, className, children, arrowClassName, variant}: DropdownProps) {
    ...
    return (
        <div className="relative">
            <Button variant={variant} onClick={() => setOpen(!open)} className={`${className} flex items-center border-0`}>{button} <FontAwesomeIcon width={10} height={10} className={`${arrowClassName} ml-2`} icon={faAngleDown} /></Button>
